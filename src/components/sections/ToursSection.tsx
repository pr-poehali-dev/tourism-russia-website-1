...
const ToursSection = () => {
  ...
  const tours: Tour[] = [
    {
      id: 1,
      title: "Коньковый поход по зимнему Байкалу (можно пешком, если нет желания на коньках)",
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filterTours(tab).map((tour) => (
                  <Card key={tour.id} className="hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 overflow-hidden">
                    ...
                    <CardHeader>
                      <CardTitle className="font-heading text-sm md:text-base">{tour.title}</CardTitle>
